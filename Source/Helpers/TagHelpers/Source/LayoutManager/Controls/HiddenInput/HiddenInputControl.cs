...
namespace RazorTechnologies.TagHelpers.LayoutManager.Controls.HiddenInput
{
    public class HiddenInputControl : BaseInputControl<HiddenInputControlVMAttribute>
    {
        ...
        public override IHtmlTagContent GetHtmlTagContent(IValueModel valueModel)
        {
            ...
            sb.AppendFormat("<input type='hidden' id='{0}' name='{1}' value='{2}' {4} placeholder='...' class='form-control' style='' {3}/>", Options.HtmlTag.UniqueId, Options.HtmlTag.Name, valueModel.Content, RenderHtmlElementDisabledAttribute(!Options.ForceDisabled), RenderHtmlElementAttribute(Options.HtmlTag.Form, Options.HtmlTag.Form));
            sb.AppendFormat("<label for='{0}' class='' style='display:none;' > {1} </label>", Options.HtmlTag.UniqueId, Options.HtmlTag.Lable);
            sb.Append(" </div>");
